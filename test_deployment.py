...
BASE_URL = 'https://people-pluse-backend-1.onrender.com'
...
def run_deployment_verification():
    ...
    try:
        resp = urllib.request.urlopen(BASE_URL + '/api/employees/?limit=5')
    ...
    try:
        req = urllib.request.Request(
            BASE_URL + f"/api/leave/approve/{leave_resp['id']}",
            data=b'',
            method='PUT',
    ...
    try:
        req = urllib.request.Request(
            BASE_URL + f"/api/leave/requests?employee_id={emps[0]['id']}",
            headers={'Authorization': f'Bearer {manager_tok}'}
        )
        resp = urllib.request.urlopen(req)
...
if __name__ == '__main__':
    sys.exit(run_deployment_verification())
