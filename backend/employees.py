...
def get_current_employee_profile(db: Session = Depends(get_db), current_user=Depends(get_current_user)):
    ...
    if not employee_id:
        if current_user.full_name:
            employee = (
                ...
                .filter(models.Employee.full_name == current_user.full_name)
                .first()
            )
        else:
            employee = None
