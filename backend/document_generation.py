...
def _build_document_context(payload: dict, employee=None, applicant=None) -> dict:
    """Return a fillable context for a document template.

    The front-end contract posts direct field values in the JSON payload, so those
    payload values must be treated as authoritative when present. The DB lookup
    is only a compatibility fallback for employee/applicant-backed workflows.
    """
    return {
...
def generate_document(payload: dict, db: Session = Depends(get_db), current_user=Depends(get_current_user)):
    ...
    # Build substitution context from the API payload first, with DB fallback only
    # for legacy find-by-id flows.
