...
def _find_headers(sheet):
    ...
    for row_index, row in enumerate(sheet.iter_rows(min_row=1, max_row=20, values_only=True), start=1):
        ...
        if any(key in normalized for key in ("file code", "employee code", "staff no", "employee id", "full name", "project", "contact", "period start", "gross", "tax", "deductions")):
...
def _find_payslip_header_row(sheet):
    best_score = -1
    best_row_index = None
    best_headers = None
    ...
    for row_index, row in enumerate(sheet.iter_rows(min_row=1, max_row=sheet.max_row, values_only=True), start=1):
        ...
        for value in normalized:
            if any(k in value for k in ("file code", "employee code", "employee id", "period start", "period end", "gross", "tax", "deduct", "net")):
                score += 1
